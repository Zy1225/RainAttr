...
A useful feature of the package for implementing bootstrap and permutation analyses is the option for **parallelization**, which allows users to reduce computation time by running replicates in parallel using the `parallel` package. This can be enabled by setting `bootstrap_opt(bootstrap_parallel = TRUE, bootstrap_parallel_num_worker = 4)` and `permutation_opt(permutation_parallel = TRUE, permutation_parallel_num_worker = 4)`, which will distributee the bootstrap and permutation runs across four workers. 
...
#### Headline Statistical Analysis in Section 5.1
...
#### After the Event Statistical Analyses in Section 5.2
